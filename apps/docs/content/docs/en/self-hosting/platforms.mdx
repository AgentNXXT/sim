...
Use the repository helper script to deploy both Sim services on Cloud Run.
...
- Builds and pushes `docker/app.Dockerfile` and `docker/realtime.Dockerfile` (without requiring a root `Dockerfile`)
- Deploys `<service-prefix>-app` and `<service-prefix>-realtime`
- Generates required auth/encryption secrets for the app service

Optional flags:
- `--service-prefix` (default `sim`)
- `--image-repo` (default `sim`)
- `--socket-url` to override socket URL
- `--copilot-api-key`

> For production, use a managed PostgreSQL database with pgvector (for example Cloud SQL + pgvector).
